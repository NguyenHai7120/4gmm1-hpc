...
{
  int i,j;
  #pragma omp parallel for private(i,j,ncol,nrow,cst,ld) shared(A)// TO BE FINISHED
  for (i=0; i<nrow; i++)
    for (j=0; j<ncol; j++)
...
double norm(int nrow, int ncol, int ld, double* A)
{
  double norm = 0.;
  int i,j;
  #pragma omp parallel for private(i,j,ncol,nrow,ld) shared(A)// TO BE FINISHED
  for (i=0; i < nrow; i++)
    for (j=0; j< ncol; j++)
...
void naive_dot(double *A, int lda, double *B, int ldb, double *C, int ldc)
{
  ...
  #pragma omp parallel for private(i,j,ldc) shared(C)// TO BE FINISHED
  for (i=0; i<M; i++)
    for (j=0; j<N; j++)
      C[ i + ldc*j ] = 0.;
  /* Perform the matrix-matrix product */
  #pragma omp parallel for private(i,j,k,lda, ldb, ldc) shared(A, B, C)// TO BE FINISHED
  for (i=0; i<N; i++)
    for (j=0; j<M; j++)
      for (k=0;  k<K; k++)
